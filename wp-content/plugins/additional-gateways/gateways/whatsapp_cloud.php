...
class WhatsappCloud
{
    ...
    public static function process_whatsapp($gateway_fields, $mobile, $message, $test_call)
    {
        ...
        $template_ids = array('template-name', 'namespace', 'language', 'has_button');
        $params_values = array();
        ...
        $has_button = true;
        if (isset($gateway_fields['has_button']) && is_numeric($gateway_fields['has_button']) && $gateway_fields['has_button'] == 0) {
            $has_button = false;
        }
        if (isset($template['has_button']) && is_numeric($template['has_button']) && $template['has_button'] == 0) {
            $has_button = false;
        }

        if (!empty($otp) && $has_button) {
            $component[] = array(
                'type' => 'button',
                'sub_type' => 'url',
                'index' => '0',
